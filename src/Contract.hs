...
data Contract = Contract
  ...
  , owner            :: Addr      -- ^ Creator of the contract
  , address          :: Addr     -- ^ Contract Address, derived during creation
  } deriving (Show, Generic, Serialize, Hash.Hashable)
...
data PermittedCallers = Anyone | Restricted (Set Addr)
...
type CallableMethods = Map.Map Name (PermittedCallers, [(Name, Script.Type)])
...
data InvalidMethodName
  ...
  deriving (Eq, Show, Generic, Serialize, Hash.Hashable)
